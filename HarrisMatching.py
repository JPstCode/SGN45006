...
def harris(im, sigma=1.0, relTh=0.0001, k=0.04):
    ...
    # Partial derivatives, get horisontal and vertical edges
...
plt.suptitle("The best 20 matches according to SSD measure", fontsize=20)
plt.imshow(montage, cmap='gray')
plt.title('The best 20 SSD matches')
...
distmat2 = np.zeros((npts1, npts2))
for i1 in range(npts1):
    for i2 in range(npts2):


        distmat2[i1,i2]=(np.sum((patches1[:,:,i1]-np.mean(patches1[:,:,i1]))*(patches2[:,:,i2]-np.mean(patches2[:,:,i2])))/
        np.sqrt(np.sum(patches1[:,:,i1]-np.mean(patches1[:,:,i1]))**2-np.sum(patches2[:,:,i2]-np.mean(patches2[:,:,i2]))**2))



ncc1 = np.amax(distmat2,axis=1)
idn1 = np.argmax(distmat2,axis=1)
ncc2 = np.amax(distmat2,axis=0)
idn2 = np.argmax(distmat2,axis=0)

pairs2 = []
for k in range(npts1):
    if k == idn2[idn1[k]]:
        pairs2.append(np.array([k, idn1[k], ncc1[k]]))
pairs2 = np.array(pairs)

sorted_ncc = np.sort(-pairs2[:,2], axis=0) # replace with your implementation
id_ncc = np.argsort(-pairs2[:,2], axis=0)  # replace with your implementation
...
plt.figure(figsize=(16, 8))
plt.suptitle("The best 20 matches according to NCC measure", fontsize=20)
plt.imshow(montage, cmap='gray')
plt.title('The best 20 NCC matches')
