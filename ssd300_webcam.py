...
while True:
    ...
    frame_copy = imresize(frame, (img_height, img_width))
    frame_copy = np.array([frame_copy])
    preds = model.predict(frame_copy)
    # [class_id, confidence, xmin, ymin, xmax, ymax]

    for pred in preds[0]:
        class_ = classes[int(pred[0])]
        xmin = int(2.13 * pred[2])
        ymin = int(1.6 * pred[3])
        xmax = int(2.13 * pred[4])
        ymax = int(1.6 * pred[5])

        if pred[1] > 0.5:
            cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), (0, 255, 0), 1)
            cv2.putText(frame, class_, (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX,
    ...
    img = cv2.putText(frame, fps, (0, 20), cv2.FONT_HERSHEY_SIMPLEX,
                      fontScale=0.8, color=(0, 255, 255))
    img = cv2.putText(frame, 'Press q to quit', (440, 20), cv2.FONT_HERSHEY_SIMPLEX,
                      fontScale=0.8, color=(0, 0, 255))
    cv2.imshow('Video feed', frame)

    # if cv2.waitKey(1) & 0xFF == ord('s'):
    #    print('saved')
    #   cv2.imwrite('ex_3.jpg',frame)
