...
u,sigma, vh = np.linalg.svd(g)

#All the rows and the first column
v_kernel = u[:,0]*np.sqrt(sigma[0])

#all the columns and the first row
h_kernel = vh[0,:]*np.sqrt(sigma[0])

gflt_imns = conv1(conv1(imns, v_kernel, axis=0, mode='reflect'), h_kernel, axis=1, mode='reflect')
gflt_imng = conv1(conv1(imng, v_kernel, axis=0, mode='reflect'), h_kernel, axis=1, mode='reflect')
...
def median_filter(img, wsize):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            ##--your-code-starts-here--##
            output[i][j] = np.median(np.sort(img_patch.ravel()))
            ##--your-code-ends-here--##
...
#when the range is increased -> filter start to work similar as gaussian filter
