...
%            A: rows, columns, color chanels. Size of the third dimension is three and it
%            represents the different color chanels which are red, green and blue.
...
% Your task: Is there any difference between each run? - Yes 
%            Why? 
%            A: Because the filters are initialised with random values
%      
%            What is the size of the output y, and how is this related to
%            x and w? 
%            A: Size of y is (380, 508, 10). The nubmer of filters
%           (w) defines the number of feature maps (10) and the original
%           input x defines the dimensions for height and width. The filter
%           goes through the whole image, but the output have slighly
%           smaller spatial dimension, because the filter wont overlap. 
%
%               
...
% A: The dimensions of height and widht are both 4 pixel bigger (2 pixel to each edge.
% The idea of padding is prevent the loss of spatial dimensions when the
% filtering is done. 
...
%            A: We can use the filter for all the color chanels.
%
%            Take a look at the result. 
%            What kind of a features does our filter extract?
%            A: Filter extracts edges from the picture     
...
% A: Because the model may need to work with data that is non-linear. Without
% the nonlinear activations the network would not outperform a single
% perceptron, because combination of the layers would just reproduce
...
%            A: Maxpooling decreases the size of the spatial dimensions, 
%            by applying a filter over the original image. It returns max 
%            value of the subregion (defined by the filter size) of the 
%            original picture. 
%            
%            What does the 'Stride' parameter do? 
%            A: Stride defines the stepsize that the filter takes when it
%            moves over the original image (dx,dy). 
...
%    A: They would not overlap at all.
%    
%    What is the result here compared to the ideal case? 
%    A: The histograms are overlapping.
...
%    Which one? 
%    A: It resembles gaussian filter


% 3. Try doubling the learning rate.
%    What is the effect of having too high of a learning rate? 
%    A: The loss values that should be used to adjust the weights of the 
%    network might start to fluctuate, which makes the model unstable.

%    Restore the learning rate and set momentum to 0.
%    How does this differ from the previous with the same learning rate?
%    A: Adding momentum causes some fluctuation in the loss values in the
%    beginning
%    What is the benefit of using momentum?
%    A: If the algorithm gets stuck in a local minimun, we can use the
%    momentum, which adds some proportion of the last step to the current
%    step and may help model to end up in the true minmum
