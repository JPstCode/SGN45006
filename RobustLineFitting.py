...
while N > sample_count:
    ...
    line_coefs = np.cross(points_h[:,a], points_h[:,b])
    ...
    for i in range(0,len(points_h[1])):
        point = points_h[:, i]
        distance = (np.abs(np.dot(line_coefs[0],point[0])+np.dot(line_coefs[1],point[1]) + line_coefs[2])/
                    np.sqrt(line_coefs[0]**2+line_coefs[1]**2))
...
##-your-code-starts-here-##
    ...
    for i in range(0,len(points_h[1])):
        point = points_h[:, i]
        distance = (np.abs(np.dot(best_line[0],point[0])+np.dot(best_line[1],point[1]) + best_line[2])/
                    np.sqrt(best_line[0]**2+best_line[1]**2))
